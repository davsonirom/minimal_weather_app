...
class WeatherService {
  //! 3.0 >  https://api.openweathermap.org/data/3.0/onecall?lat={lat}&lon={lon}&exclude={part}&appid={API key}
  //! 2.5 > https://api.openweathermap.org/data/2.5/weather?q=cidade&appid=key&unite=metric
  ...
  Future<WeatherModel> tempoParaHoje(String cidadeNome) async {
    final response = await http
        .get(Uri.parse('$BASE_URL?q=$cidadeNome&appid=$apiKey&units=metric'));

    if (response.statusCode == 200) {
      return WeatherModel.fromJson(jsonDecode(response.body));
    } else {
      throw Exception(
          'Ops! conexão com marte perdida, API fora de alcançe status = ${response.statusCode}');
    }
  }
